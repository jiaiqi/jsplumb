jsPlumb.ready(function () {
    ...
    var addNode = (window.addJsplumbNode = function (nodeId, nodeLable, position, icon) {
        debugger
        // $(`#${nodeId}`).children('.task_right').css({
        //     background: "url(" + icon + ") 0 0 no-repeat / cover",
        //     width: "20px",
        //     height: "18px",
        //     marginLeft: "5px"
        // })
        ...
        console.log("node------", $("#" + nodeId).children('.task_right'))
        $(`#${nodeId}`).children('.task_right').css({
            background: "url(" + icon + ") 0 0 no-repeat / cover",
            width: "20px",
            height: "18px",
    ...
    instance.batch(function () {
        ...
        instance.bind("beforeDrop", evt => {
            ...
            mainData.forEach(mai => {
                ...
                if (from === mai.instance_no) {
                    sourceLine = mai
                }
            })
            console.log("currLine", currLine)
            let b = currLine.in_comp.indexOf(sourceLine.comp_no)
            debugger
            if (currLine.in_instance_no.length == 0 && b >= 0) {
                ...
                if (c < 0 && b >= 0) {
                    ...
                    // Message('这是一条消息提示')
                    alert("不允许被" + sourceLine.title + "连接")
                    return false
                }
                // for (let i = 0; i < mai.in_instance_no.length; i++) {
